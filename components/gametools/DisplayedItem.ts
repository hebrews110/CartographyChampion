...
export interface DisplayedItem extends Emitter {}
export abstract class DisplayedItem {
    ...
    public async doRenderReact<T extends React.Component>(element: JSX.Element, container: HTMLElement, callback?: (component: T) => any): Promise<void> {
        GameTools.assertProperty(globalThis, "ReactDOM").render(element, container, function() {
    ...
    public static getValue<T>(item: DisplayedItem, val: GameValue<T>, container?: HTMLElement): T {
        ...
        function processReact(val: JSX.Element) {
            if(container !== undefined && item != null && item != undefined) {
                ...
                value = ((GameTools.assertProperty(globalThis, "ReactDOMServer").renderToStaticMarkup(val) as unknown) as T);
        ...
        if(GameTools.isValidReactElement(value)) {
            processReact(value);
            return undefined;
        }
    ...
    protected updateZIndex()
    {
        ...
        nodes = GameTools.assertProperty(globalThis, "_").sortBy(nodes, (node) => zindex($(node).css('z-index')));
        while (gc.firstChild) {
    ...
    private attachEventHandlers() {
        ...
        if(GameTools.ifPropertyPresent(window as any, "gt_allowCheating")) {
    ...
    private async detachReact() {
        for(const element of this.reactedSet) {
            GameTools.assertProperty(globalThis, "ReactDOM").unmountComponentAtNode(element);
...
GameTools.applyMixins(DisplayedItem, [ Emitter ]);
